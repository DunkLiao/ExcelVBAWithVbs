Attribute VB_Name = "PivotSortByValue"
' ============================================================
' PivotSortByValue.bas
' 說明：使用 Excel VBA 自動建立依值欄位排序的樞紐分析表
' 功能：
'   1. 建立新活頁簿
...
'   5. 將結果儲存至桌面
' 執行方式：在 Excel VBA 編輯器中執行 PivotSortByValue 巨集
' ============================================================

Option Explicit

' ── 常數設定 ────────────────────────────────────────────────
Const SHEET_DATA  As String = "電商訂單"
Const SHEET_PIVOT As String = "樞紐分析表"
Const PIVOT_NAME  As String = "排序樞紐"
Const OUTPUT_FILE As String = "11_PivotSortByValue.xlsx"

Sub PivotSortByValue()

    ' ── 範例資料（平台、商品類別、訂單金額）────────────────────
    Dim arrPlatforms(23)  As String
    Dim arrCategories(23) As String
    ...
    ' ── 依值欄位降冪排序列項目 ──────────────────────────────────
    ' AutoSort(Order, Field)：xlDescending=降冪，依「加總 - 訂單金額」排序
